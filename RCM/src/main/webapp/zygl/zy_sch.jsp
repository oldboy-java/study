...
function shZyTg(){
	...
	$.messager.confirm('敬告','审核通过选定记录？',function(r){
		if(r){
			$.get("/${projectName}/zygl/zy/shZysTg.action",{ids:ids},function(data){
				$('#zyGrid').datagrid('reload');
			});
		}
	});
...
function shZysBtg(){
	...
	$.messager.confirm('敬告','审核不通过选定记录？',function(r){
		if(r){
			$.get("/${projectName}/zygl/zy/shZysBtg.action",{ids:ids},function(data){
				$('#zyGrid').datagrid('reload');
			});
		}
	});
...
function updateZyDownloadAuth(ids,qx,jf){
	$.get("/${projectName}/zygl/zy/updateZyDownloadAuth.action",{zyIds:ids,auth:qx,jf:jf},function(data){
		$('#zyGrid').datagrid('reload');
	});
}
...
<table id="zyGrid" title="" fitcolumns=true
	...
	<thead>
		<tr>
			...
			<th data-options="field:'_oper',align:'center',formatter:formatZyOper">操作</th>
		</tr>
	</thead>
</table>
<%-- 查询条件设定栏 --%>
...
<div id="downloadJfSetDialog" class="easyui-dialog" style="width:400px;height:200px" data-options="title:'设置下载积分',buttons:'#tb',modal:true">
	<form id="downloadJfSetform" method="post" action="/${projectName}/zygl/xklb/updXklb.action">
		<table>
			<tr><td class='label' style="width:80px"> 积分(分)：</td><td><input id="jf" name="jf" class="easyui-numberbox" data-options="required:true,min:1,max:10000"/></td></tr>
		</table>
	</form>
</div>
<div id="tb">
	<a href="javascript:void(0)" onclick="javascript:setDownloadJf()" class="easyui-linkbutton" iconCls="icon-ok">保 存</a>
	<a href="javascript:void(0)" onclick="$('#downloadJfSetDialog').dialog('close');" class="easyui-linkbutton" iconCls="icon-remove" >取 消</a>
</div>
</body>
</html>
